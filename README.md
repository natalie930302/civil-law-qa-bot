...
推測原因:3 epoch下,語料量變大直接讓每個epoch的訓練step數等比例增加(139條約54個step,872條約327個step,增加6倍)。訓練目標(TSDAE去噪重建)跟下游任務(語義檢索排序)的不一致性沒有因為語料變多而消失,更多的訓練step等於讓模型在錯誤的方向上走得更遠、偏離原本通用語義空間更徹底。換句話說,**單純加大語料量,在固定epoch數下反而放大了災難性遺忘的破壞程度,因為它同時放大的是「訓練步數」,不是「訓練目標跟任務的一致性」**。這跟 LoRA 的結果放在一起看,更清楚地指出問題的根本原因不是「語料不夠多樣」,而是「有多少參數、被推動了多少步、往一個跟任務不一致的方向去」——凍結參數(LoRA)是對症的解法,單純加資料不是。
...
3. **驗證另一個可能的解法**:如果不用 LoRA,單純加大語料量能不能達到一樣的效果?→ 擴充語料到872條重新測試,得到「反而更差」的反直覺結果,並用「訓練step數等比例放大」的機制去解釋為什麼
...
4. 從 Hugging Face 找到可信的公開法規語料源,擴充語料到872條,驗證「單純加大語料量」這條路徑 → 得到「反而更差」的反直覺結果,並給出訓練step數放大的機制性解釋
